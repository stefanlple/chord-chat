struct PasteCommand: TextCommand {
    ...
    mutating func execute(on textEditorModel: inout TextEditorModel) {
        ...
        textEditorModel.text.insert(
            contentsOf: textEditorModel.clipboard, at: selectionRange.lowerBound)
        let newIndex = textEditorModel.text.index(
            selectionRange.lowerBound, offsetBy: textEditorModel.clipboard.count)
